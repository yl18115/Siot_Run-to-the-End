...
public class FacebookAndPlayFabManager : MonoSingleton<FacebookAndPlayFabManager> {
    ...
    [Header("PlayFab Info")] [Tooltip("ID of your application on PlayFab")] [SerializeField]
    private string _playFabTitleId;

    [Header("Facebook Share Info")] [Tooltip("Application URL")] [SerializeField]
    private string _contentUrl;

    [Tooltip("Application name")] [SerializeField]
    private string _contentTitle;

    [TextArea] [Tooltip("Message to your Facebook friends")] [SerializeField]
    private string _contentDescription;

    [Tooltip("Application image URL")] [SerializeField]
    private string _pictureUrl;

    public string PlayFabTitleId {
        get { return _playFabTitleId; }
    }
    ...
    private void SetLoggedInfo() {
        ...
        GetFacebookUserName("me", res => { FacebookUserName = res.ResultDictionary["name"].ToString(); });
    ...
    /// <param name="successCallback">Action to be executed when the process is done correctly.</param>
    /// <param name="errorCallback">Action to be executed when the process fails.</param>
    public void GetFacebookUserName(string               id, Action<IGraphResult> successCallback = null,
                                    Action<IGraphResult> errorCallback = null) {
    ...
    /// <param name="successCallback">Action to be executed when the process is done correctly.</param>
    /// <param name="errorCallback">Action to be executed when the process fails.</param>
    public void GetFacebookUserPicture(string               id, int width, int height,
                                       Action<IGraphResult> successCallback = null,
                                       Action<IGraphResult> errorCallback   = null) {
    ...
    public void GetFacebookUserPictureFromUrl(string               id, int width, int height,
                                              Action<IGraphResult> successCallback = null,
                                              Action<IGraphResult> errorCallback   = null) {
    ...
    private void PlayFabLoginSuccessCallback(PlayFab.ClientModels.LoginResult result) {
        ...
        GetFacebookUserPicture("me", PictureHeight, PictureWidth,
            res =>
            {
                FacebookUserPictureSprite =
                    Sprite.Create(res.Texture, new Rect(0, 0, PictureHeight, PictureWidth), Vector2.zero);
            });
    ...
    /// <param name="successCallback">Action to be executed when the process is done correctly.</param>
    public void UpdateStat(string                               statisticName, int value,
                           Action<UpdatePlayerStatisticsResult> successCallback = null) {
        if (!IsLoggedOnPlayFab) {
    ...
    public void GetLeaderboard(string                       statisticName,   bool friendsOnly, int maxResultsCount,
                               Action<GetLeaderboardResult> successCallback, int  startPosition = 0) {
        if (!IsLoggedOnPlayFab) {
        ...
        successCallback += res =>
        {
            Debug.Log(string.Format(
                "EasyLeaderboard.GetLeaderboard => Success! (Statistic '{0}', {1}, {2} items found)",
    ...
    /// <param name="successCallback">Action to be executed when the process is done correctly.</param>
    /// <param name="errorCallback">Action to be executed when the process fails.</param>
    public void ShareOnFacebook(Action<IShareResult> successCallback = null,
                                Action<IShareResult> errorCallback   = null) {
    ...
    private bool ValidateResult(IResult result) {
        ...
        Debug.LogError(string.Format("{0} is invalid (Cancelled={1}, Error={2}, JSON={3})",
            result.GetType(), result.Cancelled, result.Error,
            Facebook.MiniJSON.Json.Serialize(result.ResultDictionary)));
